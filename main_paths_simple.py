...
def diff_paths_random(batch_tokens, model, alphabet, num_mask, num_pairs = 20, iters = 40):
    ...
    for n in tqdm(range(num_pairs)):      
        ...
        prob_all = calc_model_prob(batch_tokens.clone(), model, alphabet, mask_idxs, idx_i) # mask all at once
        prob_all = torch.log(prob_all[0,idx_i]) # The log-probability for the specific sequence we are mutating
        ...
        prob_all_at_once[n] = np.sum([prob_all[i, new_vals[i]].item() - prob_all[i, old_vals[i]].item() for i in range(num_mask)])
